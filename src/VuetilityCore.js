...
export default class Vuetilitycore {
     ...
     * @return Object
     */
    computed(model, modelName, nameSpace = null, isUnique = false) {
        let computed = {}

        for (let [key, value] of Object.entries(model)) {
            let type = VLib.get(value, 'computedType', 'default')
            let computedName = key
            // unique models get a unique key name
            if (isUnique) {
                computedName = modelName + '__' + key
            }
            computed[computedName] = this[type + 'Computed'].call(
                this,
                key,
                modelName,
                value,
    ...
    computedSetter(
        ...
        basicComponent.componentScope.$store.commit(
            this._mutationName(nameSpace),
            {
                model: modelName,
                key,
                data: checkedValue,
    ...
    propertyValue(key, modelName, modelProperty, nameSpace) {
        ...
        if (propertyValue === false) {
            if (modelProperty.defaultValue !== undefined) {
                this.componentScope.$store.commit(
                    this._mutationName(nameSpace),
                    {
                        model: modelName,
                        key: key,
